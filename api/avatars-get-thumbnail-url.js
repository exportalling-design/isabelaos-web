...
export default async function handler(req, res) {
  try {
    ...
    if (!avatar.ref_image_path) {
      return res.json({ ok: true, avatar_id, name: avatar.name, status: avatar.status, thumb_url: null });
    }

    const { data: signed, error: sErr } = await supabase.storage
      .from(bucket)
      .createSignedUrl(avatar.ref_image_path, expires_in);
    ...
    return res.json({
      ...
      ref_image_path: avatar.ref_image_path,
      thumb_url: signed?.signedUrl || null,
      expires_in
    });
  } catch (err) {
    console.error("[avatars-get-thumbnail-url]", err);
