// api/comercial-poll-status.js
// Mismo patrón que api/cineai/poll-status.js — comercial-generate.js solo
// lanza el job a EvoLink y devuelve taskId; este endpoint hace el polling.
...
async function uploadToStorage(buf, userId, jobId) {
  const filePath = `${userId}/comercial_${jobId}_${Date.now()}.mp4`;
...
export default async function handler(req, res) {
  ...
  try {
    ...
    console.log(`[comercial-poll] EvoLink taskId=${taskId} status=${status} progress=${data.progress}`);
    ...
    if (!evolinkVideoUrl) {
      console.error(`[comercial-poll] NO VIDEO URL:`, JSON.stringify(data));
      return res.status(200).json({ ok: true, status: "processing" });
    }
    ...
    console.error("[comercial-poll] error:", err.message);
