...
async function saveVideo(userId, videoUrl, taskId) {
  try {
    ...
    console.error("[poll] saved:", path);
    return data?.publicUrl || videoUrl;
  } catch (err) {
    console.error("[poll] saveVideo failed:", err.message, "— URL original como fallback");
    return videoUrl;
  }
}
...
export default async function handler(req, res) {
  ...
  let result;
  try {
    result = provider === "evolink_seedance"
      ? await pollEvolink(providerTask)
      : await pollByteplus(providerTask);
  } catch (err) {
    console.error(`[poll] ${provider} error:`, err.message);
    return res.status(200).json({ ok: true, status: "processing", jobId: job.id });
  }

  console.error(`[poll] ${provider} done:${result.done} failed:${result.failed} video:${result.videoUrl ? "yes" : "no"}`);
  ...
  if (result.done && result.videoUrl) {
    ...
    console.error("[poll] done sin videoUrl. raw:", JSON.stringify(result.rawData || {}).slice(0, 600));
    newStatus = "FAILED";
