// api/generate-img2video.js ✅ CON COLA REAL + LIBERA SLOT
...
export default async function handler(req, res) {
  try {
    console.log("[GEN_I2V] VERSION 2026-02-02c");
    ...
    if (lockErr) {
      return res.status(200).json({ ok: true, job_id: jobId, queued: true, reason: lockErr.message });
    }

    if (!canDispatch) {
      return res.status(200).json({ ok: true, job_id: jobId, queued: true, reason: "no_slot_available" });
    }
