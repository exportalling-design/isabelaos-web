...
//
// ✅ CAMBIOS IMPORTANTES (anti-traba / anti-perdida de job):
// 1) Creamos job en Supabase ANTES del dispatch => SIEMPRE aparece en video_jobs.
// 2) Actualizamos status: QUEUED -> DISPATCHED -> IN_PROGRESS/RUNNING (según worker)
// 3) Si dispatch falla => status ERROR y mensaje amigable (para UI).
//
...
async function ensureWorkingPod(sb, log) {
  ...
  if (!statePodId) {
    ...
    if (!running.ok)
      throw new Error(`Pod nuevo no llegó a RUNNING: ${running.error || running.status}`);
...
// -----------------------------------------------------------
// Buscar job activo del usuario (para no crear otro)
// -----------------------------------------------------------
async function getActiveJobForUser(sb, user_id, mode) {
  ...
  // Opcional: filtrar por modo (t2v/i2v)
  const { data, error } = mode ? await q.eq("mode", mode) : await q;
  if (error) throw error;
  return data?.[0] || null;
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const dispatchUrl = `${workerUrl}${WORKER_ASYNC_PATH}`;

    // ✅ 3) Crear job en Supabase ANTES del dispatch (ANTI “no aparece running”)
    ...
    const payloadToStore = {
      ...
      fps,
      guidance_scale,
      image_base64: image_base64 ? "[base64]" : null,
    };

    // NOTA: guardamos el payload real (con base64) también, pero si es gigante,
    // mejor guardarlo aparte. Si tu tabla ya aguanta, podés guardar el base64 completo.
    ...
    const rr = await fetch(dispatchUrl, {
      ...
      body: JSON.stringify({ job_id, ...payloadToWorker }),
    });

    const tt = await rr.text().catch(() => "");
    if (!rr.ok) {
      const errMsg = `Worker dispatch failed (${rr.status}): ${tt.slice(0, 400)}`;
      ...
      // opcional: si dispatch falla feo, pod pudo quedar inútil
      // await runpodTerminatePod(podId).catch(() => {});

      return res.status(200).json({
        ok: false,
        status: "DISPATCH_FAILED",
        job_id,
        error: "No se pudo iniciar el worker. Intenta de nuevo.",
        debug: errMsg,
        using_optimized: usingOptimized,
        used_prompt: finalPrompt,
        used_negative_prompt: finalNegative,
    ...
    // El worker puede responder vacío o con status; igual dejamos IN_PROGRESS
    let workerReply = null;
    try {
      workerReply = tt ? JSON.parse(tt) : null;
    } catch {
      workerReply = { raw: tt?.slice(0, 500) || "" };
    }
