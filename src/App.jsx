...
</div>  <p className="mt-2 text-xs text-neutral-400">    
      Usa tu correo o entra con Google para acceder al motor de producción    
      visual.    
    </p>    <form onSubmit={handleSubmit} className="mt-4 space-y-3">    
  <div>    
...
<h2 className="text-lg font-semibold text-white">Motor de imagen · Producción visual</h2>  {isDemo && (
...
className={group relative overflow-hidden rounded-xl border ${     selected && selected.id === item.id ? "border-cyan-400" : "border-white/10"     } bg-black/60}
>
<img    
src={item.src}    
alt="Generación"    
...
{new Date(item.createdAt).toLocaleString()}
  ...
  </div>      <div className="rounded-3xl border border-white/10 bg-black/40 p-6 flex flex-col">    
...
para publicación.  
</p>  <div className="mt-4 rounded-2xl border border-white/10 bg-black/50 px-4 py-2 text-xs text-neutral-300">    
    ...
    </div>    <div className="mt-4 space-y-4 text-sm">    
  <div>    
    <label className="text-neutral-300">Prompt</label>    
    <textarea    
      className="mt-1 h-24 w-full resize-none rounded-2xl bg-black/60 px-3 py-2 text-sm text-white outline-none ring-1 ring-white/10 focus:ring-2 focus:ring-cyan-400"    
...
</p>  <div className="mt-5 space-y-4 text-sm">    
      <div>    
...
const packs = useMemo(
() => [
{ id: "jade_50", title: "Pack 50 Jades", jades: 50, price: "9.00" },
{ id: "jade_100", title: "Pack 100 Jades", jades: 100, price: "15.00" },
{ id: "jade_300", title: "Pack 300 Jades", jades: 300, price: "39.00" },
],
[]
);

const handlePaddleJadesCheckout = async (pack) => {
if (!user?.id) return;
setError("");
try {
setBusy(true);
...
console.error("Paddle jades error:", e);
...
const creditJadesWithPayPal = async (details, pack) => {
if (!user?.id) return;
setError("");
try {
setBusy(true);
...
<div className="rounded-3xl border border-yellow-400/30 bg-yellow-500/5 p-6 text-center text-sm text-yellow-100">  
Inicia sesión para acceder a tu wallet de jades.  
</div>  
);  
}  return (

<div className="rounded-3xl border border-white/10 bg-black/40 p-6">  
<h2 className="text-lg font-semibold text-white">Wallet · Jades</h2>  <div className="mt-4 rounded-2xl border border-white/10 bg-black/50 px-4 py-3 text-xs text-neutral-300">    
    <div className="flex flex-wrap items-center justify-between gap-2">    
    ...
    onClick={onRefresh}    
    disabled={busy}    
    className="rounded-xl border border-white/20 px-3 py-1.5 text-[11px] text-white hover:bg-white/10 disabled:opacity-60"    
  >    
    Actualizar    
  </button>    
</div>

  </div>    {error && <p className="mt-3 text-xs text-red-400 whitespace-pre-line">{error}</p>}

  <div className="mt-6 grid gap-4 md:grid-cols-3">    
    {packs.map((pack) => (    
      <div key={pack.id} className="rounded-3xl border border-white/10 bg-black/50 p-5">    
        ...
        </div>    <p className="mt-2 text-[11px] text-neutral-400">Créditos para generación en módulos de video.</p>    
    ...
    <div className="mt-4">    
      ...
      <PayPalButton    
        ...
        containerId={`paypal-${pack.id}`}    
        onPaid={(details) => creditJadesWithPayPal(details, pack)}    
      />    

      <button    
        type="button"    
        onClick={() => handlePaddleJadesCheckout(pack)}    
        disabled={busy}    
        className="mt-3 w-full rounded-2xl border border-white/15 bg-white/5 px-4 py-2 text-xs font-semibold text-white hover:bg-white/10 disabled:opacity-60"    
      >    
  ...
  </div>      <p className="mt-5 text-[11px] text-neutral-400">    
    Si ya pagaste y no ves el saldo actualizado, usa “Actualizar”.    
  </p>    
</div>  );
}
...
<div className="text-[10px] text-neutral-500">Panel del creador · Beta</div>  
...
</div>  <div className="flex items-center gap-3 text-xs">    
        <span className="hidden sm:inline text-neutral-300">    
          {user?.email} {isAdmin && "· admin"}    
        </span>    <div className="hidden md:flex items-center gap-2 rounded-2xl border border-white/10 bg-black/60 px-3 py-1.5">    
  ...
  </header>      <main className="mx-auto max-w-6xl px-4 pb-16 pt-10">    
    <div className="mb-4 md:hidden">    
      ...
      </div>    <p className="text-[11px] font-semibold text-neutral-300 mb-2">Navegación</p>    
...
<section className="flex gap-6">    
  ...
  <div className="flex-1 space-y-6">    
    <div>    
      ...
      <p className="mt-1 text-xs text-neutral-400">    
        Controla tu flujo de producción visual: genera, revisa, descarga y administra resultados desde un    
        solo sistema conectado a GPU.    
      </p>    
    </div>    
...
<div className="text-[10px] text-neutral-500">Motor de producción visual</div>  
    ...
    <section className="grid gap-10 lg:grid-cols-[1.4fr_1fr]">    
      <div>    
        ...
        </p>    <h1 className="mt-3 text-4xl font-semibold leading-tight md:text-5xl">    
