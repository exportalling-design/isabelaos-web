...
//
// Features:
//   - Selector modo: TikTok Trends / Escena Cinematográfica
//   - Banner de celebridades bloqueadas (copyright)
//   - Upload foto del usuario (I2V / R2V+face)
//   - Modo "Animar foto exacta" — mantiene escenario original
//   - Upload video de referencia para copiar movimiento (R2V)
//   - Selector duración 5s / 10s / 15s con costo en Jades
//   - Selector formato 9:16 / 16:9 / 1:1
//   - Prompt libre con presets por modo
//   - Bloqueo celebridades en tiempo real (frontend + backend)
//   - Modal "¿Cómo funciona?" en español
//   - Polling automático cada 4s
//   - Extracción último frame para continuar escena
//   - Vista en grande del video con X para cerrar
//   - Botón descarga directa
//   - Guarda en video_jobs → aparece en biblioteca
//
// Imports: mismo path que usan los demás componentes
// ─────────────────────────────────────────────────────────────
...
// ── Nombres bloqueados (validación rápida en frontend) ────────
const BLOCKED_PREVIEW = [
  ...
  "bruce lee","brucelle","bruce willis","jackie chan","arnold schwarzenegger",
];
...
// ── Extraer último frame de un video con canvas ───────────────
...
// ── Texto del modal "¿Cómo funciona?" ────────────────────────
const HOW_IT_WORKS = [
  {
    icon: "🕺",
    title: "TikTok Trends",
    desc: "Selecciona el tipo de trend (baile, lip sync, comedy, etc). Si subes un video del trend que quieres replicar, el modelo copia el movimiento exacto. Si subes tu foto, tu cara aparece en el video.",
  },
  {
    icon: "🎬",
    title: "Escena Cinematográfica",
    desc: "Elige el tipo de escena (acción, pelea, drama, épico, noir). El modelo genera una escena de calidad Hollywood. Puedes subir tu foto para aparecer en la escena.",
  },
  {
    icon: "👤",
    title: "Tu foto (opcional)",
    desc: "Sube una foto tuya (de frente, buena iluminación) y el modelo usará tu cara como personaje principal. Sin foto, genera un personaje original según tu descripción.",
  },
  {
    icon: "🖼️",
    title: "Animar foto exacta",
    desc: "Activa esta opción cuando quieras animar una foto sin que el modelo cambie el escenario, personajes u objetos. Ideal para fotos grupales o escenas específicas que ya tienes.",
  },
  {
    icon: "🎥",
    title: "Video de referencia",
    desc: "Sube el video del baile o movimiento que quieres replicar. El modelo copia la coreografía exacta fotograma a fotograma. Si también subiste tu foto, tu cara ejecuta ese movimiento.",
  },
  {
    icon: "▶",
    title: "Continuar escena",
    desc: "Cuando termina un video, el botón 'Continuar escena' extrae el último fotograma automáticamente y lo usa como punto de partida para el siguiente clip. Así puedes encadenar varios videos y crear escenas de 1-2 minutos.",
  },
  {
    icon: "🚫",
    title: "Celebridades bloqueadas",
    desc: "Por derechos de autor no puedes generar videos con rostros de celebridades (Tom Cruise, Bad Bunny, Messi, etc.) ni personajes de estudios (Spider-Man, Batman, Darth Vader, etc.). El sistema lo bloquea automáticamente. Puedes describir un personaje original con características similares.",
  },
  {
    icon: "💎",
    title: "Costo en Jades",
    desc: "5 segundos = 40 Jades · 10 segundos = 75 Jades · 15 segundos = 110 Jades. Los Jades se descuentan al iniciar la generación. Si hay un error del servidor, se reembolsan automáticamente.",
  },
];
...
export default function CineAIPanel() {
  ...
  // ── Opciones ─────────────────────────────────────────────
  const [animateExact, setAnimateExact] = useState(false); // animar foto exacta
  ...
  const uploadToStorage = async (file, folder, setUrl, setPreview, setUploading) => {
    ...
    try {
      ...
      setError("Error subiendo archivo. Verifica el bucket 'user-uploads' en Supabase.");
    } finally {
      setUploading(false);
    }
  };
  ...
  const startPolling = useCallback((taskId) => {
    ...
    pollRef.current = setInterval(async () => {
      ...
      } catch { /* silencioso — seguir intentando */ }
    }, 4000);
  }, []);
    ...
    try {
      ...
      const res = await fetch("/api/cineai/generate", {
        ...
        body: JSON.stringify({
          ...
          animateExact: animateExact && !!faceImageUrl && !isContinuation,
          duration,
          aspectRatio:  ratio,
    ...
    <div className="cp">
      ...
      <div className="cp-header">
        <div className="cp-header-row">
          <div>
            ...
            <p className="cp-tagline">Escenas cinematográficas y trends virales con IA · Seedance 2.0</p>
      ...
      <div className="blocked-banner-top">
        ...
        <span>
          ...
          No puedes generar videos con Tom Cruise, Bad Bunny, Messi, Spider-Man, Batman, etc.
          Describe un personaje original con características similares.
        </span>
      </div>
      ...
      <div className="cp-grid">
            ...
            {videoUrl && (
              <>
                ...
                <div className="result-actions">
                  {/* Ver en grande */}
                  <button className="ra-btn" onClick={() => setVideoFullscreen(true)}>
                    ⛶ Ver en grande
                  </button>
                  {/* Descargar */}
        ...
        <div className="cp-cell">
          ...
          {isContinuation && lastFrameUrl ? (
            // Modo continuación — muestra el frame extraído
            ...
            <>
              ...
              >
                {facePreview ? (
                  <>
                    ...
                    <img src={facePreview} className="uz-thumb" alt="preview" />
                  </>
                ) : uploadingFace ? (
              ...
              {facePreview && (
                <>
                  <div
                    className={`animate-toggle ${animateExact ? "active" : ""}`}
                  ...
                  {animateExact && (
                    <p className="animate-toggle-desc">
                      El modelo animará tu foto sin cambiar el escenario, personajes u objetos. Ideal para fotos grupales o escenas específicas.
                    </p>
                  )}
                </>
              )}
        ...
        <div className="cp-cell cp-cell-full">
          ...
          {liveBlocked && (
            <div className="blocked-banner">
              <span>🚫</span>
              <span>
                <strong>"{liveBlocked}"</strong> está bloqueado por derechos de autor.
                Describe un personaje original con características similares.
              </span>
            </div>
          )}
