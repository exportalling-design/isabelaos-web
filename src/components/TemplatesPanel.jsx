...
// ── TEMPLATES ──────────────────────────────────────────────────────────────
// Videos: public/gallery/divine-light.mp4, divine-human.mp4, couple-disaster.mp4, victorias-secret.mp4
const TEMPLATES = [
  {
    id: "divineLight",
    emoji: "⚡",
    ...
    description: { es: "Enfrenta a Dios como ser de luz. Tormenta. Tsunami. Redención.", en: "Confront God as a being of light. Storm. Tsunami. Redemption." },
    ...
    slots: { male: ["protagonist"], female: ["protagonist"] },
  },
  {
    id: "divineHuman",
    emoji: "🙏",
    ...
    description: { es: "Dios como ser humano real. Túnica blanca. La misma tormenta.", en: "God as a real human. White robe. Same storm." },
  ...
  },
  {
    ...
    description: { es: "Una pareja en un acantilado. Meteorito. Tsunami los separa.", en: "A couple on a cliff. Meteor. Tsunami separates them." },
    ...
    genderOptions: [
      ...
      { value: "both",   icon: "👫", label: { es: "Somos los Dos", en: "We're Both" }, sub: { es: "2 fotos de rostro", en: "2 face photos" } },
    ],
    slots: { female: ["protagonist"], male: ["protagonist"], both: ["man", "woman"] },
  },
  {
    id: "victoriasSecret",
    emoji: "👙",
    ...
    description: { es: "Campaña de moda de lujo. Playa tropical. Solo mujer.", en: "Luxury fashion campaign. Tropical beach. Women only." },
    video: "/gallery/victorias-secret.mp4",
    accent: "#C8A96E",
    genderOptions: null,
    slots: { female: ["protagonist"] },
  },
];

const SLOT_LABELS = {
  protagonist: { es: "Tu foto de rostro", en: "Your face photo" },
  man:         { es: "Foto del Hombre",   en: "Man's photo" },
  woman:       { es: "Foto de la Mujer",  en: "Woman's photo" },
};
...
// ── PROFILE SHEET BOX — muestra la hoja generada ──────────────────────────
function ProfileBox({ slot, accent, lang, label }) {
  if (slot.generatingProfile) {
    return (
      <div style={{
        border: `2px dashed ${accent}66`, borderRadius: 14, minHeight: 150,
        ...
        <div style={{ fontSize: 12, color: accent, fontFamily: "'Syne',sans-serif", fontWeight: 700 }}>
          {lang === "es" ? "Generando con Gemini..." : "Generating with Gemini..."}
        </div>
        <div style={{ fontSize: 10, color: "rgba(255,255,255,0.3)" }}>
          {lang === "es" ? "Esto toma ~10 seg" : "Takes ~10 sec"}
        </div>
      </div>
    );
  }

  if (slot.profilePreview) {
    return (
      <div style={{ borderRadius: 14, overflow: "hidden", border: `1.5px solid ${accent}66` }}>
        <img src={slot.profilePreview} alt="Profile sheet"
          style={{ width: "100%", height: 150, objectFit: "cover", display: "block" }} />
        <div style={{
          padding: "7px 10px", background: "rgba(0,0,0,0.75)",
          fontSize: 11, color: accent, fontFamily: "'Syne',sans-serif", fontWeight: 700,
          display: "flex", alignItems: "center", gap: 6,
        }}>
          {slot.fallback
            ? <>⚠️ {lang === "es" ? "Usando foto original" : "Using original photo"}</>
            : <>✓ {lang === "es" ? "Hoja de referencia lista" : "Reference sheet ready"}</>
          }
        </div>
      </div>
    );
  ...
  return (
    <div style={{
      border: "2px dashed rgba(255,255,255,0.08)", borderRadius: 14, minHeight: 150,
      display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 6,
    }}>
      <div style={{ fontSize: 28, opacity: 0.15 }}>🎭</div>
      <div style={{ fontSize: 10, color: "rgba(255,255,255,0.2)", textAlign: "center", padding: "0 12px", lineHeight: 1.5 }}>
        {lang === "es" ? "Hoja de referencia\nse genera automáticamente" : "Reference sheet\nauto-generated on upload"}
      </div>
    </div>
  );
}
  ...
  return (
    <div onClick={onClick}
      ...
      <div style={{
        ...
        fontFamily: "'Syne',sans-serif", fontWeight: 800, fontSize: 9, letterSpacing: 1.5,
        ...
        {tmpl.genderOptions
          ? tmpl.genderOptions.map((g) => (
            <div key={g.value} style={{
              ...
              border: "1px solid rgba(255,255,255,0.15)", fontSize: 10,
              fontFamily: "'Syne',sans-serif", fontWeight: 600, color: "rgba(255,255,255,0.85)",
...
export default function TemplatesPanel({ userJades = 0, onJadesUpdate }) {
  ...
  // Slots: protagonist / man / woman
  const emptySlot = () => ({ file: null, preview: null, profileData: null, profilePreview: null, generatingProfile: false, fallback: false });
  ...
  // Active slot keys for current template + gender
  const activeSlotKeys = () => {
    if (!tmpl || !genderVariant) return [];
    return tmpl.slots[genderVariant] || [];
  };
  ...
  // ── Upload face → generate profile sheet ─────────────────────────────────
  const handleFaceUpload = async (slotKey, file) => {
    const preview = URL.createObjectURL(file);
    setSlots((prev) => ({
      ...prev,
      [slotKey]: { file, preview, profileData: null, profilePreview: null, generatingProfile: true, fallback: false },
    }));
    setErrorMsg("");
    try {
      ...
      // On error, use the original photo as fallback so user can still generate
      const b64 = await fileToBase64(file).catch(() => null);
      setSlots((prev) => ({
        ...prev,
        [slotKey]: {
          ...prev[slotKey],
          profileData: b64 ? { base64: b64, mimeType: file.type } : null,
          profilePreview: b64 ? preview : null,
          generatingProfile: false,
          fallback: true,
        },
      }));
      setErrorMsg(lang === "es" ? "Gemini no disponible — usando foto original como referencia" : "Gemini unavailable — using original photo as reference");
    }
  };

  // ── Upload body → generate body profile ──────────────────────────────────
  const handleBodyUpload = async (file) => {
    setBodyFile(file);
    setBodyPreview(URL.createObjectURL(file));
    setBodyProfileData(null);
    setBodyProfilePreview(null);
    setGeneratingBodyProfile(true);
    try {
      const b64 = await fileToBase64(file);
      const json = await callApi("generate-profile", { faceBase64: b64, faceMime: file.type, isFace: false });
      setBodyProfileData(json.profile);
      setBodyProfilePreview(`data:${json.profile.mimeType};base64,${json.profile.base64}`);
    } catch (e) {
      const b64 = await fileToBase64(file).catch(() => null);
      if (b64) {
        setBodyProfileData({ base64: b64, mimeType: file.type });
        setBodyProfilePreview(URL.createObjectURL(file));
      }
    } finally {
      setGeneratingBodyProfile(false);
    }
  };
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const json = await callApi("submit-video", {
        templateId: selectedId, lang, quality, genderVariant,
    ...
    const iv = setInterval(async () => {
      try {
        ...
        if (json.status === "completed" || json.status === "succeed") {
          ...
          throw new Error(lang === "es" ? "La generación falló" : "Generation failed");
        }
  ...
  const resetAll = () => {
    ...
    setBodyFile(null); setBodyPreview(null); setBodyProfileData(null); setBodyProfilePreview(null);
  ...
  if (view === "gallery") return (
    <div style={{ fontFamily: "'Inter',sans-serif", color: "#fff", paddingBottom: 60 }}>
      ...
      <div style={{ padding: "28px 20px 0", display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
        <div>
          <div style={{ fontFamily: "'Syne',sans-serif", fontSize: 22, fontWeight: 800 }}>
            🎬 {lang === "es" ? "Plantillas Épicas" : "Epic Templates"}
          </div>
      ...
      <div style={{
        ...
        border: "1px solid rgba(200,169,110,0.2)", borderRadius: 14, padding: "14px 16px",
      }}>
        <div style={{ fontFamily: "'Syne',sans-serif", fontSize: 13, fontWeight: 700, marginBottom: 3 }}>
          {lang === "es" ? "✨ Elige una escena → sube tu foto → la IA te pone en ella" : "✨ Choose a scene → upload your photo → AI places you in it"}
        </div>
        <div style={{ fontSize: 11, color: "rgba(255,255,255,0.4)", lineHeight: 1.6 }}>
          {lang === "es"
            ? "La IA genera una hoja de referencia de personaje con tu rostro y la usa para colocarte en la escena."
            : "AI generates a character reference sheet from your face and uses it to place you in the scene."}
        </div>
      </div>
      ...
      <div style={{ padding: "14px 20px 0", fontSize: 10, color: "rgba(255,255,255,0.18)", textAlign: "center" }}>
        {lang === "es" ? "Hover para previsualizar · 15 segundos · 9:16" : "Hover to preview · 15 seconds · 9:16"}
      </div>
    </div>
  );
    ...
    <div style={{ fontFamily: "'Inter',sans-serif", color: "#fff", paddingBottom: 80 }}>
      ...
      {tmpl?.genderOptions && (
        <div style={{ padding: "20px 20px 0" }}>
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            {tmpl.genderOptions.map((g) => (
              <div key={g.value} onClick={() => {
                setGenderVariant(g.value);
                setSlots({ protagonist: emptySlot(), man: emptySlot(), woman: emptySlot() });
              }} style={{
                flex: 1, minWidth: 90, border: `1.5px solid ${genderVariant === g.value ? accent : "rgba(255,255,255,0.1)"}`,
                borderRadius: 12, padding: "12px 10px", cursor: "pointer", textAlign: "center",
                background: genderVariant === g.value ? `${accent}18` : "rgba(255,255,255,0.03)",
                transition: "all 0.2s",
              }}>
      ...
      {genderVariant && (
        <div style={{ padding: "20px 20px 0" }}>
          ...
          <div style={{
            ...
            📌 {lang === "es"
              ? "Foto frontal · Sin lentes · Sin accesorios · Sin filtros · Buena iluminación · Fondo simple"
              : "Front photo · No glasses · No accessories · No filters · Good lighting · Simple background"}
          </div>

          {currentSlots.map((slotKey) => (
            <div key={slotKey} style={{ marginBottom: 16 }}>
              {currentSlots.length > 1 && (
                <div style={{ fontSize: 11, color: "rgba(255,255,255,0.5)", marginBottom: 8, fontFamily: "'Syne',sans-serif", fontWeight: 700 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                ...
                <div>
                  ...
                  <UploadZone
                    label={lang === "es" ? "Subir foto de rostro" : "Upload face photo"}
                    hint={lang === "es" ? "Foto frontal clara sin lentes" : "Clear front photo, no glasses"}
                ...
                <div>
                  <div style={{ fontSize: 10, color: "rgba(255,255,255,0.3)", marginBottom: 6 }}>
                    {lang === "es" ? "Hoja de referencia (Gemini)" : "Reference sheet (Gemini)"}
                  </div>
                  <ProfileBox slot={slots[slotKey]} accent={accent} lang={lang} label={SLOT_LABELS[slotKey][lang]} />
                </div>
              </div>
            </div>
          ))}
        ...
        <div style={{ padding: "0 20px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
            <div style={{ fontSize: 10, fontWeight: 600, color: accent, textTransform: "uppercase", letterSpacing: 1.5 }}>
              {tmpl?.genderOptions
                ? (lang === "es" ? "3 · Referencia de cuerpo" : "3 · Body reference")
                : (lang === "es" ? "2 · Referencia de cuerpo" : "2 · Body reference")}
            </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
            <UploadZone
              label={lang === "es" ? "Foto de cuerpo" : "Body photo"}
            ...
            <div>
              <div style={{ fontSize: 10, color: "rgba(255,255,255,0.3)", marginBottom: 6 }}>
                {lang === "es" ? "Hoja de cuerpo (Gemini)" : "Body sheet (Gemini)"}
              </div>
              <ProfileBox
                slot={{ generatingProfile: generatingBodyProfile, profilePreview: bodyProfilePreview, profileData: bodyProfileData, fallback: false }}
                accent={accent} lang={lang} label="body"
              />
            </div>
          </div>
        </div>
      ...
      {genderVariant && (
        <div style={{ padding: "18px 20px 0" }}>
          <div style={{ fontSize: 10, fontWeight: 600, color: accent, textTransform: "uppercase", letterSpacing: 1.5, marginBottom: 10 }}>
            {tmpl?.genderOptions
              ? (lang === "es" ? "4 · Calidad del video" : "4 · Video quality")
              : (lang === "es" ? "3 · Calidad del video" : "3 · Video quality")}
          </div>
          <div style={{ display: "flex", gap: 10 }}>
            {[
              { q: "480", label: "480p", cost: 30, desc: { es: "Estándar · Rápido", en: "Standard · Fast" } },
              { q: "720", label: "720p HD", cost: 60, desc: { es: "Alta calidad · Más lento", en: "High quality · Slower" } },
            ].map((o) => (
              <div key={o.q} onClick={() => setQuality(o.q)} style={{
                flex: 1, border: `1.5px solid ${quality === o.q ? accent : "rgba(255,255,255,0.1)"}`,
                borderRadius: 12, padding: "12px 14px", cursor: "pointer",
                background: quality === o.q ? `${accent}14` : "rgba(255,255,255,0.03)", transition: "all 0.2s",
              }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
                  <span style={{ fontFamily: "'Syne',sans-serif", fontWeight: 800, fontSize: 13 }}>{o.label}</span>
                  <span style={{ fontFamily: "'Syne',sans-serif", fontWeight: 800, fontSize: 13, color: accent }}>{o.cost} 💎</span>
                </div>
      ...
      {(step === "error" || errorMsg) && (
        <div style={{
          ...
          ⚠️ {errorMsg}
          {step === "error" && (
            <button onClick={() => { setStep("idle"); setErrorMsg(""); }} style={{
      ...
      {step === "polling" && (
        <div style={{
          margin: "14px 20px 0", background: `${accent}0a`,
          border: `1px solid ${accent}33`, borderRadius: 14, padding: "20px", textAlign: "center",
        }}>
      ...
      {step !== "done" && genderVariant && (
        <div style={{ padding: "18px 20px 0" }}>
          <button onClick={handleGenerate} disabled={!canGenerate} style={{
            ...
            {step === "submitting"
              ? (lang === "es" ? "⏳ Enviando a PiAPI..." : "⏳ Submitting to PiAPI...")
              : anyGenerating
              ? (lang === "es" ? "⚙️ Generando hojas de referencia..." : "⚙️ Generating reference sheets...")
              : `✨ ${lang === "es" ? "Generar Mi Video" : "Generate My Video"} · ${jadeCost} 💎`}
          </button>

          {/* Helper text */}
          {!currentSlots.some((k) => slots[k]?.file) && (
            <div style={{ fontSize: 11, color: "rgba(255,255,255,0.25)", textAlign: "center", marginTop: 8 }}>
              {lang === "es" ? "↑ Sube tu foto de rostro primero" : "↑ Upload your face photo first"}
            </div>
          )}
          {currentSlots.some((k) => slots[k]?.file) && !allProfilesReady && !anyGenerating && (
            <div style={{ fontSize: 11, color: "rgba(255,255,255,0.25)", textAlign: "center", marginTop: 8 }}>
              {lang === "es" ? "⚙️ Espera a que se generen las hojas de referencia" : "⚙️ Wait for reference sheets to generate"}
            </div>
          )}
      ...
      {!genderVariant && tmpl?.genderOptions && (
        <div style={{ padding: "16px 20px 0", fontSize: 12, color: "rgba(255,255,255,0.3)", textAlign: "center" }}>
          {lang === "es" ? "↑ Selecciona quién eres en la escena para continuar" : "↑ Select who you are in the scene to continue"}
        </div>
      )}
    </div>
  );
